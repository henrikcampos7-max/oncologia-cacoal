...
| A04 | **Fronteira do serviço de pseudonimização precisa ser formalizada:** O documento já descreve corretamente que o de-para fica em sistema separado e que nenhum PII trafega na entidade `Paciente`. A melhoria necessária é formalizar essa fronteira em ADR: definir que o serviço separado tem controle de acesso independente, que o acesso ao mapeamento reverso é restrito a perfil específico (ex.: DPO/responsável designado) e que a API principal é proibida de retornar o mapeamento reverso em qualquer resposta. | ARQUITETURA_MVP.md §3.1 Paciente | Registrar ADR formalizando: (a) fronteira e responsabilidade do serviço separado de de-para, (b) perfil com acesso ao mapeamento reverso e mecanismo de controle, (c) restrição explícita na API principal de nunca expor o de-para. |
...
| M03 | **Ausência de critério de aceitação para importação de XLSX com erro parcial:** A Seção 5.2 (testes de importação) menciona "rejeitar linhas inválidas com relatório de erros; não alterar estoque em importações com erro", mas não há critério claro do que constitui "erro" (linha, arquivo inteiro, lote de linhas). | ARQUITETURA_MVP.md §5.2 Importação | Definir: importação é atômica (tudo ou nada) ou admite sucesso parcial com relatório? Documentar critério de validação por linha. |
...
| M05 | **Saldo negativo deve ser prevenido por restrição transacional, não somente por alerta:** RC01 proíbe saldo negativo, mas não há requisito explícito de restrição de integridade técnica (ex.: constraint no banco ou validação pré-commit na transação) que impeça a persistência de saldo negativo caso o controle de aplicação falhe. Um alerta de saldo negativo seria tardio e insuficiente — a condição não deveria ser persistida. | ARQUITETURA_MVP.md §3.4 RC01, entidade `Movimentacao` | Adicionar requisito de que a saída de estoque seja rejeitada por validação transacional antes de persistir quando `disponivel(medicamento_id) < quantidade_solicitada`. Se uma inconsistência for detectada (ex.: via concorrência), deve gerar evento operacional/de segurança (não alerta de rotina) e bloquear novas saídas do medicamento até reconciliação humana. |
...
| B05 | **Requisitos de segurança de API dependentes de stack ainda não definido:** Sem decisão de stack (D01), prescrever mecanismos específicos (ex.: JWT, OAuth) é prematuro e pode ser descartado. O risco é a implementação de autenticação sem modelo de ameaças documentado. | ARQUITETURA_MVP.md §4, D01 | Antes da Fase 1, conduzir modelo de ameaças básico e documentar controles compatíveis com a autenticação escolhida: gestão segura de sessão (expiração, invalidação), proteção CSRF quando houver cookies, limitação de tentativas de autenticação, autorização verificada por operação (não só por perfil global) e registro de eventos de autenticação na auditoria. |
